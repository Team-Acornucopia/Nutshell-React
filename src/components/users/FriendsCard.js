        ...
        return (
            <Message key={this.props.user.id} className="card">
                <h5 className="card-title">
                    <Message.Header>Friend's Name: {this.props.user.username}</Message.Header>
                    <Button as={Link} size="tiny" color="purple" className="nav-link" to={`/friends/${this.props.user.id}`}>Details</Button>
                    <Button size="tiny" color="red"
                        onClick={() => this.props.deleteFriend(thisSpecificFriend, currentUser)}
